(function(){
  ...
  function roundLabel(){const meta=window.roomState?.meta||{};const idx=Number(meta.roundIndex??-1)+1;const total=(window.missionCount?window.missionCount():0)||Number(meta.totalRounds||0)||0;return idx>0?`Ronde ${idx}/${total||'?'}`:'Menunggu ronde';}
  function teamCount(){
    try{
  ...
  function refreshStudentUi(){
    ...
    ensureStructure();
    const meta=window.roomState?.meta||{};
    ...
    const initial=getInitial((window.currentUserName||window.getStudent?.()?.name||'K'));
    const roleArea=$('studentRoleSummaryArea');
    if(roleArea){
      roleArea.innerHTML=`<div class="student-role-summary"><div class="student-role-avatar" style="background:${roleColor(roleName)}">${escapeHtml(initial)}</div><div><div class="student-role-badge">${escapeHtml(roleName)}</div><h3>${escapeHtml(window.getStudent?.()?.name||'Kamu')} ${window.getStudent?.()?'(kamu)':''}</h3><p>${escapeHtml(roleDesc)}</p></div></div><div class="student-role-note">Dengarkan semua pendapat, jaga diskusi tetap fokus pada soal, dan pastikan keputusan diambil bersama tim.</div>`;
    }
    const taskList=$('studentRoleTaskList');
    if(taskList){
      const tasks=buildTaskList(roleName);
      let activeCount=0,totalCount=0;
      try{const entries=window.currentRoundChatEntries?.()||[];const st=window.getStudent?.();const students=window.getStudents?.()||{};const teammates=Object.entries(students).filter(([,x])=>st&&x.groupId===st.groupId);totalCount=teammates.length;const counts={};entries.forEach(([,x])=>counts[x.uid]=(counts[x.uid]||0)+1);activeCount=teammates.filter(([uid])=>(counts[uid]||0)>0).length;}catch(_){ }
      taskList.innerHTML=tasks.map((t,i)=>`<li class="${i===tasks.length-1?'done':''}">${escapeHtml(t)}</li>`).join('');
      const rs=$('studentRoleStatus');
      if(rs){rs.classList.toggle('hidden',!totalCount); if(totalCount)rs.textContent=`${activeCount}/${totalCount} anggota sudah aktif`;}
    }
    ...
    // restyle chat messages into avatar + bubble
    const box=$('groupChatMessages');
    if(box){
      const nodes=Array.from(box.children);
      nodes.forEach(node=>{
        if(node.classList.contains('chat-empty')||node.classList.contains('chat-message')) return;
        if(node.dataset && node.dataset.enhanced==='1') return;
        const name=node.querySelector?.('.name')?.textContent?.trim() || node.querySelector?.('.chat-name')?.textContent?.trim() || '';
        const time=node.querySelector?.('.time')?.textContent?.trim() || node.querySelector?.('.chat-time')?.textContent?.trim() || '';
        const text=node.querySelector?.('.text')?.textContent?.trim() || node.querySelector?.('.chat-text')?.textContent?.trim() || textOnly(node.innerHTML);
        const wrap=document.createElement('div');wrap.className='chat-message';
        wrap.innerHTML=`<div class="chat-avatar">${escapeHtml(getInitial(name))}</div><div><div class="meta"><div class="name">${escapeHtml(name||'Anggota')}</div><div class="time">${escapeHtml(time)}</div></div><div class="bubble">${escapeHtml(text)}</div></div>`;
    ...
    const answerArea=$('studentAnswerArea');
    if(answerArea){
