...
(function(){
  ...
  function renderStudent(){
    ...
    if(review&&review.index===idx){
      ...
      sideHtml=`<aside class="concept-study-question concept-inline-feedback ${ok?'correct':'wrong'}"><span class="concept-question-kicker">HASIL JAWABAN</span><div class="concept-feedback-icon">${ok?'✓':'!'}</div><h2>${ok?'Jawaban tepat':'Belum tepat'}</h2><p class="concept-feedback-answer">Jawaban benar: <b>${esc(item.options[item.correct])}</b></p><div class="concept-rationale"><b>Mengapa?</b><p>${esc(item.rationale)}</p></div><button type="button" class="btn primary concept-next-btn" onclick="DMConceptGame.next()">${idx+1>=TOTAL?'Lihat Hasil Akhir':'Lanjut ke Misi Berikutnya'} →</button></aside>`;
    }else{
      sideHtml=`<aside class="concept-study-question"><div class="concept-question-side-head"><span class="concept-question-kicker">SOAL PEMAHAMAN</span><strong>Misi ${idx+1}/${TOTAL}</strong></div><h2>${esc(item.question)}</h2><div class="concept-options concept-options-side">${item.options.map((op,i)=>`<button type="button" ${answerBusy?'disabled':''} onclick="DMConceptGame.answer(${i})"><span>${String.fromCharCode(65+i)}</span><b>${esc(op)}</b></button>`).join('')}</div><div class="concept-question-note">Pilih satu jawaban yang paling tepat. Penjelasan dan animasi tetap terlihat di sebelah kiri agar kamu dapat membandingkannya sebelum menjawab.</div></aside>`;
    }

    body.innerHTML=`<article class="concept-study-shell"><div class="concept-study-grid"><section class="concept-study-explain"><div class="concept-q-head"><span class="concept-topic-pill ${item.topic}">${esc(topic)}</span><strong>Misi ${idx+1}/${TOTAL}</strong></div><span class="concept-question-kicker">PELAJARI PELAN-PELAN</span><h2>${esc(item.title)}</h2><p class="concept-explain-copy">${esc(item.explain)}</p><div class="concept-read-tip"><b>Tips belajar:</b> perhatikan contoh dan animasi langkah demi langkah. Setelah merasa paham, jawab soal pada panel di sebelah kanan.</div>${visualHtml(item)}<div class="concept-explain-actions"><button type="button" class="btn secondary" onclick="DMConceptGame.replay()">↻ Ulangi animasi</button></div></section>${sideHtml}</div></article>`;
  }
