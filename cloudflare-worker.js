...
async function ensureFourSurpriseCards(caseItem, idx, env) {
  const existing = normalizeSurpriseCards(caseItem)
  if (existing.length === 4) {
    caseItem.surpriseCards = existing
    return caseItem
  }

  const slides = Array.isArray(caseItem?.slides) ? caseItem.slides : []
  const caseText = [
    `Judul: ${clean(caseItem?.title || `Ronde ${idx + 1}`, 140)}`,
    `Pertanyaan utama: ${clean(caseItem?.text || '', 500)}`,
    ...slides.map((s, i) => `Slide ${i + 1} - ${clean(s?.title || '', 120)}: ${clean(s?.text || '', 900)}`)
  ].join('\n')

  const prompt = `Buat tepat 4 opsi Kartu Kejutan untuk satu kasus diskusi Informatika kelas VIII.

KASUS:
${caseText}

Kartu Kejutan adalah perkembangan BARU setelah kasus awal berlangsung.
Keempat kartu wajib:
- sangat terkait dengan tokoh, platform, tindakan, data, perangkat, atau kejadian yang memang ada dalam kasus;
- berbeda secara substantif;
- cukup penting sehingga jawaban awal kelompok perlu disesuaikan;
- tidak mengubah topik menjadi kasus lain;
- tidak memberi jawaban final;
- bukan pilihan ganda.

Variasikan fungsi 4 kartu:
1. bukti/informasi baru,
2. konsekuensi atau risiko baru,
3. pihak lain yang ikut terdampak,
4. kendala, bantahan, atau perubahan situasi.

Keluarkan JSON murni:
{"surpriseCards":[
  {"id":"slug-singkat","title":"judul singkat","condition":"1-2 kalimat perkembangan baru yang konkret dan spesifik terhadap kasus","instruction":"1 kalimat hal baru yang harus dipertimbangkan dalam jawaban"}
]}`
  ...
  if (cards.length !== 4) throw new Error(`Kartu Kejutan Ronde ${idx + 1} belum lengkap.`)
  ...
  "cases": [
    {
      ...
      "surpriseCards": [
        {"id":"slug-singkat","title":"judul perkembangan baru","condition":"1-2 kalimat kondisi baru yang sangat terkait dengan kasus","instruction":"1 kalimat hal yang sekarang harus dipertimbangkan dalam jawaban"}
      ]
    }
  ]
}
...
- SETIAP KASUS wajib memiliki tepat 4 surpriseCards.
- Empat Kartu Kejutan harus dibuat dari kasus yang SAMA, bukan template generik.
- Setiap kartu berupa perkembangan baru setelah kejadian pada slide: bukti/informasi baru, konsekuensi/risiko baru, pihak lain terdampak, atau kendala/bantahan/perubahan situasi.
- Gunakan tokoh, platform, benda, akun, data, perangkat, atau tindakan yang memang muncul pada kasus tersebut.
- Keempat kartu harus berbeda secara substantif dan cukup penting untuk mengubah, memperkuat, atau memprioritaskan ulang jawaban kelompok.
- Kartu tidak boleh memberi jawaban final dan tidak boleh berupa pilihan ganda.
...
- Hindari jawaban terlalu panjang.`
  ...
  // V19: 4 Kartu Kejutan diselesaikan saat paket kasus dibuat.
  for (let i = 0; i < Math.min(rounds, list.length); i++) {
    list[i] = await ensureFourSurpriseCards(list[i], i, env)
  }
  ...
  for (let i = 0; i < cases.length; i++) {
    if (!Array.isArray(cases[i].surpriseCards) || cases[i].surpriseCards.length !== 4) {
      throw new Error(`Kasus Ronde ${i + 1} belum memiliki 4 Kartu Kejutan.`)
    }
  }

  while (cases.length < rounds) {
    let padded = { title: `Ronde ${cases.length + 1}`, text: 'Diskusikan kasus ini bersama tim.', slides: [] }
    padded = await ensureFourSurpriseCards(padded, cases.length, env)
    cases.push(normalizeCase(padded, cases.length, normalizedCfg))
  }
...
// Legacy endpoint: V19 tidak memakainya saat ronde; kartu sudah dibuat bersama kasus.
