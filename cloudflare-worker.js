...
Pastikan jumlah slides pada SETIAP objek tepat ${slidesPerCase}.`.trim();
  ...
  try {
    ...
    if (!Array.isArray(parsed)) return json({ error: "Format kasus dari Gemini tidak valid. Silakan generate ulang." }, 502, origin);
    if (parsed.length !== rounds) return json({ error: `Gemini menghasilkan ${parsed.length} kasus, seharusnya ${rounds}. Silakan generate ulang.` }, 502, origin);

    const cases = [];
    for (let i = 0; i < parsed.length; i++) {
      const item = parsed[i] || {};
      const slidesRaw = Array.isArray(item.slides) ? item.slides : [];
      const optionsRaw = Array.isArray(item.options) ? item.options : [];
      if (slidesRaw.length !== slidesPerCase || optionsRaw.length !== 3) {
        return json({ error: `Struktur kasus ronde ${i + 1} belum sesuai. Silakan generate ulang.` }, 502, origin);
      }
      const slides = slidesRaw.map((x, j) => ({
        title: clean(x?.title || `Bagian ${j + 1}`, 100),
        text: clean(x?.text || "", 1200)
      }));
      if (slides.some(x => x.text.length < 60)) {
        return json({ error: `Bacaan ronde ${i + 1} terlalu singkat. Silakan generate ulang.` }, 502, origin);
      }
      const options = optionsRaw.map(x => clean(x, 240));
      if (options.some(x => !x)) return json({ error: `Opsi ronde ${i + 1} tidak lengkap. Silakan generate ulang.` }, 502, origin);
      let correct = item.correct;
      if (correct !== null) {
        correct = Number(correct);
        if (![0, 1, 2].includes(correct)) return json({ error: `Kunci ronde ${i + 1} tidak valid. Silakan generate ulang.` }, 502, origin);
      }
      cases.push({
