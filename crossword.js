/* DIGITAL MISSION — TTS Kelompok v1
...
(function(){
  ...
  const ROLE_LABELS={reader:'Pembaca Petunjuk',proposer:'Pemberi Usulan',checker:'Pemeriksa',writer:'Penulis Jawaban'};
  ...
  function initTeamState(){return {activeEntryId:'',proposal:null,votes:{},attempts:{},solved:{},score:0,finishedAt:0,updatedAt:now()}}
  ...
  function rolesFor(gid,entryId){
    const members=teamMembers(gid),entry=entriesArray().find(x=>x.id===entryId),offset=Number(entry?.order||0),roles={reader:[],proposer:[],checker:[],writer:[]};
    if(!members.length)return roles;
    Object.keys(roles).forEach((role,i)=>{const owner=members[(offset+i)%members.length];roles[role].push(owner[0])});
    return roles;
  }
  function myRoleKeys(gid,entryId){
    const uid=user()?.uid,assign=rolesFor(gid,entryId);return Object.keys(assign).filter(k=>assign[k].includes(uid));
  }
  function roleOwnerName(gid,entryId,role){
    const uid=rolesFor(gid,entryId)[role]?.[0],st=students()[uid];return st?.name||'-';
  }
  function requiredVotes(gid){
    const n=teamMembers(gid).length,mode=cw().config?.consensus||'all';
    if(n<=1)return 1;return mode==='majority'?Math.floor(n/2)+1:n;
  }
  ...
  async function activateEntry(entryId){
    ...
    await ref.transaction(t=>{
      ...
      t.activeEntryId=entryId;t.proposal=null;t.votes={};t.updatedAt=Date.now();return t;
    });
  }
  ...
  async function submitProposal(){
    ...
    const mine=myRoleKeys(st.groupId,entryId);
    if(!mine.includes('proposer'))return alert('Pada soal ini, usulan awal dikirim oleh Pemberi Usulan. Sampaikan idemu melalui chat kepada anggota yang mendapat peran tersebut.');
    ...
    const path=`${roomBase()}/crossword/teams/${st.groupId}`;
    const updates={proposal:{text,uid:user().uid,name:st.name||'Siswa',createdAt:now()},votes:{[user().uid]:true},updatedAt:now()};
    await db().ref(path).update(updates);if(input)input.value='';
    await sendSystemChat(`Usulan jawaban: ${text}. Anggota tim silakan periksa dan beri persetujuan.`);
  }
  async function voteYes(){
    const st=student(),team=currentTeam(),entryId=team?.activeEntryId;if(!st?.groupId||!entryId||!team?.proposal)return;
    await db().ref(`${roomBase()}/crossword/teams/${st.groupId}/votes/${user().uid}`).set(true);
  }
  async function discussAgain(){
    const st=student();if(!st?.groupId)return;
    await db().ref(`${roomBase()}/crossword/teams/${st.groupId}`).update({proposal:null,votes:{},updatedAt:now()});
    await sendSystemChat('Tim memilih mendiskusikan ulang jawaban sebelum dikirim.');
  }
  async function submitAnswer(){
    ...
    const mine=myRoleKeys(st.groupId,entryId);if(!mine.includes('writer'))return alert('Jawaban final hanya dapat dimasukkan oleh Penulis Jawaban pada soal ini.');
    const votes=Object.values(team.votes||{}).filter(Boolean).length,need=requiredVotes(st.groupId);
    if(votes<need)return alert(`Belum cukup persetujuan. Diperlukan ${need} anggota.`);
    ...
    await ref.transaction(t=>{
      ...
      if(proposed===correct){
        const pts=Math.max(10,20-(attempts-1)*5);t.solved=t.solved||{};t.solved[entryId]={answer:correct,points:pts,solvedAt:Date.now(),writerUid:user().uid};t.score=Number(t.score||0)+pts;t.activeEntryId='';t.proposal=null;t.votes={};
        if(Object.keys(t.solved).length>=entriesArray().length)t.finishedAt=Date.now();outcome=`correct:${pts}`;
      }else{t.proposal=null;t.votes={};outcome='wrong'}
    ...
    else if(outcome==='wrong')await sendSystemChat('Jawaban belum tepat. Diskusikan kembali sebelum mencoba lagi.');
  }
  ...
  function renderStudentChat(){
    ...
    host.innerHTML=list.length?list.map(m=>{
      ...
      return `<div class="cw-chat-message ${me?'me':''}"><div class="cw-chat-avatar">${esc((m.name||'?').slice(0,1).toUpperCase())}</div><div class="cw-chat-body"><b>${esc(m.name||'Siswa')}${me?' (kamu)':''}</b><time>${esc(time)}</time><div class="cw-chat-bubble">${esc(m.text||'')}</div></div></div>`;
  ...
  function renderActiveClue(st,team,puzzle,state){
    ...
    if(!entry){zone.className='cw-clue-zone waiting';zone.innerHTML='<strong>Pilih satu soal untuk dibahas bersama</strong><p>Klik nomor pada papan atau daftar soal. Setelah dipilih, seluruh anggota tim akan fokus pada soal yang sama.</p>';return}
    zone.className='cw-clue-zone';
    const roles=rolesFor(st.groupId,entry.id),myKeys=myRoleKeys(st.groupId,entry.id),proposal=team.proposal,votes=team.votes||{},voteCount=Object.values(votes).filter(Boolean).length,need=requiredVotes(st.groupId),canPropose=myKeys.includes('proposer'),canWrite=myKeys.includes('writer');
    zone.innerHTML=`<span class="cw-clue-label">Soal Aktif</span><div class="cw-clue-title"><span class="cw-clue-no">${entry.number}</span><strong>${entry.direction==='H'?'Mendatar':'Menurun'} · ${entry.answer.length} huruf</strong></div><p class="cw-clue-text">${esc(entry.clue)}</p><div class="cw-role-row">${Object.keys(ROLE_LABELS).map(k=>`<div class="cw-role-card ${roles[k]?.includes(user()?.uid)?'me':''}"><span>${esc(ROLE_LABELS[k])}</span><b>${esc(roleOwnerName(st.groupId,entry.id,k))}</b></div>`).join('')}</div><div class="cw-action-zone">${proposal?`<div class="cw-proposal-current"><div><small>Usulan ${esc(proposal.name||'anggota')}</small><strong>${esc(proposal.text)}</strong></div><span class="cw-inline-status ${voteCount>=need?'good':''}">${voteCount}/${need} persetujuan</span></div><div class="cw-votes">${teamMembers(st.groupId).map(([uid,s])=>`<span class="cw-vote ${votes[uid]?'yes':''} ${uid===user()?.uid?'me':''}">${esc(s.name)} ${votes[uid]?'✓':'○'}</span>`).join('')}</div><div class="cw-submit-row"><button class="btn secondary" type="button" onclick="window.DMCrossword.voteYes()" ${votes[user()?.uid]?'disabled':''}>${votes[user()?.uid]?'Sudah setuju':'Setuju'}</button><button class="btn secondary" type="button" onclick="window.DMCrossword.discussAgain()">Diskusikan lagi</button><button class="btn primary" type="button" onclick="window.DMCrossword.submitAnswer()" ${canWrite&&voteCount>=need?'':'disabled'}>Masukkan ke TTS</button><span class="cw-inline-status">${canWrite?'Kamu Penulis Jawaban.':'Penulis: '+esc(roleOwnerName(st.groupId,entry.id,'writer'))}</span></div>`:`<div class="cw-proposal-line"><input id="cwProposalInput" maxlength="16" placeholder="Usulan jawaban…" ${canPropose?'':'disabled'} onkeydown="if(event.key==='Enter')window.DMCrossword.submitProposal()"><button class="btn primary" type="button" onclick="window.DMCrossword.submitProposal()" ${canPropose?'':'disabled'}>Ajukan</button></div><span class="cw-inline-status">${canPropose?'Kamu Pemberi Usulan pada soal ini.':'Pemberi usulan: '+esc(roleOwnerName(st.groupId,entry.id,'proposer'))}</span>`}</div>`;
  }
  ...
  window.DMCrossword={
    ...
    activateEntry,submitProposal,voteYes,discussAgain,submitAnswer,sendChat,buildPuzzle
  };
})();
