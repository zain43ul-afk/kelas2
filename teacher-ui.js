(function(){
  ...
  function renderPreview(){
    ...
    box.innerHTML=slides.map((sl,i)=>{
      ...
      const image=sharedImage?`<img src="${escAttr(sharedImage)}" alt="Ilustrasi utama ronde" loading="lazy">`:`<div class="teacher-ai-preview-placeholder"><span>${i+1}</span></div>`;
  ...
  async function generateCases(){
    ...
    const n=$('teacherAiInlineNotice');if(n){n.className='teacher-ai-inline-notice wait';n.textContent='AI sedang membuat paket kasus dan 1 ilustrasi utama per ronde…'}
    try{
      ...
      if(n){n.className='teacher-ai-inline-notice good';n.textContent=`Paket kasus berhasil diperbarui dengan ${Number(result?.imageCount||0)} ilustrasi utama (1 per ronde).`}
