(function(){
  ...
  function openScores(){
    const total=Number(bridge().getMissionCount?.()||0);
    const finalDone=isLocked()&&total>0&&roundIndex()>=total-1;
    if(finalDone){smoothFocus($('teacherFinalScoresCard'));return}
    toggleAnswers(true);
  }
  ...
  async function generateCases(){
    ...
    try{
      ...
      if(n){n.className='teacher-ai-inline-notice good';n.textContent=`Paket kasus berhasil diperbarui dengan ${Number(result?.imageCount||0)} ilustrasi.`}
  ...
  window.DMTeacherUI={refresh,switchView,switchActivePanel,toggleAnswers,toggleCaseViewer,toggleAnalysis,focusLeaderboard,openScores,updateDateChip,setDifficulty,generateCases,resetAiForm,setMode};
