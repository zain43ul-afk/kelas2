  ...
  async function bootstrap(){
    ...
    if(DM_EMBEDDED){
      try{
        ...
        document.body.classList.remove("dm-excel-boot");
        document.body.innerHTML=`<main style="max-width:720px;margin:70px auto;padding:30px;font-family:system-ui"><div style="padding:24px;border:1px solid #efcaca;border-radius:18px;background:#fff7f7;color:#7d2f2f"><h2 style="margin-top:0">Lab Informatika tidak dapat dibuka</h2><p>${escapeHtml(err.message||String(err))}</p><button onclick="sessionStorage.removeItem('dm-selected-game');location.href='../'" style="padding:10px 16px;border:0;border-radius:10px;background:#2563eb;color:#fff;font-weight:800">Kembali ke Digital Mission</button></div></main>`;
      }
      return;
    }
