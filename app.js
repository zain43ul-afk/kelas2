...
(function(){
  ...
  function renderTeacher(){
    ...
    if(progress){
      progress.innerHTML=Object.entries(groups()).length?Object.entries(groups()).map(([gid,g])=>{
        ...
        return `<div class="cw-team-progress-row"><div class="cw-team-name-cell"><b>${esc(g.name||gid)}</b><small class="cw-team-member-names">${memberNames}</small></div><small>${contrib} jawaban tim</small><div><div class="cw-mini-track"><i style="width:${pct}%"></i></div><small>${active&&!globalSolved[t.activeEntryId]?`Membahas ${active.number} ${active.direction==='H'?'Mendatar':'Menurun'}`:'Menunggu soal'}</small></div><b>${Number(t.score||0)} pt</b></div>`;
  ...
  window.DMCrossword={
    ...
    activateEntry,submitProposal,setProposalDraft,submitReview,discussAgain,submitAnswer,sendChat,kickStudent,buildPuzzle
  };
})();
