...
(function(){
  ...
  const FALLBACK=[
    ...
    ['DEEPFAKE','Video atau suara hasil rekayasa AI yang meniru seseorang.','Wajah seseorang tampak mengatakan hal yang tidak pernah ia ucapkan.'],
  ...
  const LOCAL_HELP_BY_ANSWER=new Map(FALLBACK.map(([answer,,help])=>[answer,help]));
  ...
  function normalizeCandidates(raw){
    ...
    return list.map((x,i)=>{
      ...
      // Bantuan selalu berasal dari bank lokal atau pola huruf, bukan dari respons AI.
  ...
  async function generateTeacherPuzzle(){
    ...
    try{
      ...
      const res=await fetch(worker,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:'generate_crossword',crossword:{topic,difficulty,count:Math.max(25,target+25)}})});
    ...
    try{
      // Kata lokal melengkapi hasil AI saat tema menghasilkan terlalu sedikit persilangan.
      const bank=FALLBACK.map(([answer,clue])=>({answer,clue}));
      const puzzle=buildPuzzle([...candidates,...shuffled(bank)],target,title,topic);
  ...
  function renderTeamHints(team,puzzle,state){
    ...
    const records=HINT_KIN.filter(([kind])=>used[kind]).map(([kind,label])=>{
    ...
    host.innerHTML=`<div class="cw-hints-heading"><strong>Bantuan Tim</strong><span>${remaining}/3 tersisa</span></div>
      <p>Petunjuk langsung dari bank lokal atau huruf jawaban, tanpa AI. Tiap bantuan dapat dipakai sekali untuk soal aktif dan terlihat oleh seluruh tim.</p>
  ...
  window.DMCrossword={
    ...
    activateEntry,useHint,submitProposal,setProposalDraft,submitReview,discussAgain,submitAnswer,sendChat,kickStudent,buildPuzzle
  };
})();
